...
#if !defined(LIGHTCLEAR_INC)
#define LIGHTCLEAR_INC
...
{
  #ifdef FORWARD_BASE_PASS
    clip(-1); // clip base pass to make invisible while avoiding artifacts
  #endif
  ...
  // we'll need the light's info before PBS
  UnityLight light = CreateLight(i);

  // get light instensity to determine visibility
  float br = max3(light.color.rgb);
  // cache original light color
  float4 rimColor = float4(light.color, 1);
  // change color to white to not tint the revealed appearance
  ...
  // generate blend factor for rim color
  float blend = max(alpha - 0.5, 0)*2;
  // apply rim color via lerp
  col = alpha < 1 ? lerp(rimColor, col, pow(blend, 3)) : col;

  // fade revealed color out
  col.a = alpha*alpha;
