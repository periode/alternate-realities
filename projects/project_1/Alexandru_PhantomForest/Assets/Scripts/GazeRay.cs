...
public class GazeRay : MonoBehaviour
{
    ...
    public ViewTrigger[] defaultViewTriggers;
	public AnyViewTrigger[] otherViewTriggers;
    ...
    void Update()
    {
        ...
        foreach (var trigger in defaultViewTriggers)
        {
            trigger.Focused = hit.collider && (hit.collider.gameObject == trigger.gameObject);
        }
		foreach (var trigger in otherViewTriggers)
		{
			trigger.Focused = hit.collider && (hit.collider.gameObject == trigger.gameObject);
		}
    }
